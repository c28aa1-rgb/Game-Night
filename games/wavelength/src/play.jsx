...
function App() {
  ...
  useEffect(() => {
    ...
    emit('join_team', { code: joined.code, team: joined.team, token: joined.token }).then((result) => {
  ...
  async function join(code, team) {
    ...
    const result = await emit('join_team', { code, team, token: remembered?.team === team ? remembered.token : '' });
    if (result.error) return setError(result.error);
    const next = { code, team, token: result.token };
...
function Join({ code: initialCode, connected, onJoin, error }) {
  ...
  function teamButton(team) {
    ...
    const mine = remembered?.team === team;
    const occupied = Boolean(teamState?.occupied);
    const unavailable = occupied && !mine;
    const ready = connected && roomCode.length >= 4 && teams !== null && !unavailable;
    const label = unavailable ? `${teamName(team)} already joined` : mine ? `Rejoin ${teamName(team)}` : `Join ${teamName(team)}`;
    const sublabel = unavailable ? 'Use the open team' : teamState?.connected ? 'This phone can rejoin' : mine ? 'Saved on this phone' : 'Available';
    return <button key={team} className={unavailable ? 'team-choice team-choice--taken' : 'team-choice'} disabled={!ready} onClick={() => onJoin(roomCode, team)}><b>0{team + 1}</b>{label}<small>{sublabel}</small></button>;
  }
  return <main className="play join-screen"><a className="back-link" href="/wavelength">&larr; Game night</a><header><span className="readout">TEAM CONTROLLER</span><h1>WAVELENGTH</h1><p>Choose the phone your team will share.</p></header>
    ...
    <p className="join-team-status" aria-live="polite">{roomCode.length < 4 ? 'Enter the room code to see team availability.' : teams === null ? 'Checking team availability…' : 'A joined team is locked to its original phone.'}</p>
    <div className="join-teams">{[0, 1].map(teamButton)}</div>
...
function Controller({ state, privateState, emit, error, setError }) {
  ...
  else if (state.phase === 'clue' && active) panel = <Clue state={state} privateState={privateState} act={act} />;
  else if (state.phase === 'guess' && active) panel = <Guess state={state} privateState={privateState} act={act} />;
  ...
  else panel = <Wait state={state} act={act} text={state.phase === 'clue' ? `${teamName(state.activeTeam)} is finding the target.` : `${teamName(state.activeTeam)} is moving the marker.`} />;
  return <main className="play"><header className="play__bar"><a href="/wavelength" className="wordmark">WAVELENGTH</a><span className="readout">{teamName(privateState.team)} · {state.code}</span></header>
    <div className="phone-score"><span>TEAM 1 <b>{state.scores[0]}</b></span><span>TEAM 2 <b>{state.scores[1]}</b></span></div>
    <AnimatePresence mode="wait"><motion.section key={`${state.phase}-${active}-${state.turn}-${privateState.team}`} className="phone-panel" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.24, ease: [0.2, 0, 0, 1] }}>{panel}</motion.section></AnimatePresence>
    {error && <p className="error error--fixed">{error}</p>}
  </main>;
}

function Setup({ state, act }) {
  return <><span className="readout">ROOM SETUP</span><h1>Calibrate the night.</h1><p>Both teams can set the session. The first valid control wins.</p><div className="round-stepper"><button onClick={() => act('set_rounds', { rounds: state.roundCount - 1 })} disabled={state.roundCount <= 3}>&minus;</button><div><b>{state.roundCount}</b><span>ROUNDS</span></div><button onClick={() => act('set_rounds', { rounds: state.roundCount + 1 })} disabled={state.roundCount >= 10}>+</button></div>
    <div className="ready-row">{state.teams.map((team, index) => <span key={index} className={team.connected ? 'ready' : ''}>{teamName(index)} {team.connected ? 'READY' : 'WAITING'}</span>)}</div><button className="primary" disabled={!state.teams.every((team) => team.connected)} onClick={() => act('start_game')}>Start calibration</button></>;
}

function Clue({ state, privateState, act }) {
  return <><span className="readout">EYES ONLY · YOUR TEAM'S TURN</span><h1>Read the signal.</h1><p>Give one clue aloud. Never show this target to your team.</p><div className="private-spectrum"><SpectrumArc markerIndex={privateState.targetIndex} targetIndex={privateState.targetIndex} privateView lowLabel={state.prompt.low} highLabel={state.prompt.high} /></div><button className="primary" onClick={() => act('clue_given')}>I gave the clue — pass phone</button></>;
}
...
function Guess({ state, privateState, act }) {
  ...
  return <><span className="readout">TEAM {privateState.team + 1} &middot; CALIBRATE</span><h1>Where does the clue land?</h1><p>The TV marker moves with every press. Talk it through, then lock it.</p><div className="controller"><button className="arrow" aria-label="Move marker left" onPointerDown={(event) => start(-1, event)} onPointerUp={stop} onPointerCancel={stop} onLostPointerCapture={stop}>&larr;</button><div className="position"><b>{state.markerIndex + 1}</b><span>OF 61</span></div><button className="arrow" aria-label="Move marker right" onPointerDown={(event) => start(1, event)} onPointerUp={stop} onPointerCancel={stop} onLostPointerCapture={stop}>&rarr;</button></div><button className="primary primary--lock" onClick={() => act('lock_guess')}>Lock this signal</button></>;
}

function Wait({ state, act, text, button, event = 'advance' }) { return <><span className="readout">{state.tiebreaker ? 'SUDDEN DEATH' : 'STANDBY'}</span><h1>{text}</h1><p>{button ? 'Either team phone can continue when the room is ready.' : 'Watch the shared screen and wait for the handoff.'}</p>{button && <button className="primary" onClick={() => act(event)}>{button}</button>}</>; }
