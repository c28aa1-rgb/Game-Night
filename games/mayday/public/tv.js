 ...
 * new beat it fires the whole sequence — klaxon flash, alarm tone, hard cut to
 * silence, narrator, cutscene, role reveal — in that order, every time.
 *
...
async function loadJoinInfo(code) {
  ...
  try {
    const res = await fetch(`/mayday/api/join-info?code=${encodeURIComponent(code)}`);
...
Narrator.onLine((text, kind, meta) => {
  // The alarm dies the moment the voice starts — not when the line was queued.
  ...
  if (kind === 'intro' && meta) Cutscene.introScene(meta.index);
});
...
async function handleBeat(beat) {
  switch (beat.kind) {
    ...
    case 'morning_vent':
      await alarmThen('morning_vent', { name: beat.victim.name });
      playDeath(beat.victim, 'vent', false);
      break;
    ...
    case 'vote_out': {
      say('vote_closes');
      // The tally gets a moment of its own before the airlock opens.
      await sleep(1500);
 ...
 * discussion, and render() runs before the beat does — so without this the
...
function playDeath(victim, method, harsh) {
  Cutscene.play({
    ...
    victim,
    harsh,
    // The narrator's tag is part of the reveal — the glyph alone does not land it.
    onTag: (role) => say('role_tag', { role }),
  });
}
...
const PHASE_LABEL = {
  lobby: 'Awaiting crew',
  intro: 'Distress relay',
  night_intro: 'Night falls',
  night_saboteurs: 'Saboteurs move',
  night_sensor: 'Sensor sweep',
  night_medic: 'Medic rounds',
...
const SCREEN_FOR = {
  ...
  night_saboteurs: 'screen-night',
  night_sensor: 'screen-night',
  night_medic: 'screen-night',
...
const NIGHT_VIEW = {
  intro: { glyph: NIGHT_GLYPH, tint: 'var(--klaxon)', title: 'Kestrel', note: 'Distress relay running.' },
  deal: {
    glyph: Cutscene.GLYPHS.crew.svg,
    tint: 'var(--amber)',
  ...
  night_saboteurs: { glyph: Cutscene.GLYPHS.saboteur.svg, tint: 'var(--klaxon)', title: 'Saboteurs', note: 'Saboteurs are choosing. Everyone else, eyes closed.' },
  night_sensor: { glyph: Cutscene.GLYPHS.sensor.svg, tint: 'var(--terminal)', title: 'Sensor sweep', note: 'One scan is being taken. Everyone else, eyes closed.' },
  night_medic: { glyph: Cutscene.GLYPHS.medic.svg, tint: '#E8F3F7', title: 'Medic rounds', note: 'One door is being sealed. Everyone else, eyes closed.' },
};
...
function render() {
  ...
  el('hud-cycle').textContent = phase === 'lobby' ? 'Standby'
    : phase === 'game_over' ? 'Game over'
      : phase === 'intro' ? 'Kestrel · last transmission'
        : dealing ? 'Crew manifest' : `Cycle ${state.cycle}`;
...
function renderLobby() {
  ...
  const rows = [
    ['Sensor Officer', config.sensor],
    ['Medic', config.medic],
    ['Security Officer', config.security],
    ['Mimic', config.mimic && players.length >= 7],
  ...
  el('setup-brief').textContent = enough
    ? `${players.length} crew · ${state.saboteurCount} saboteur${state.saboteurCount === 1 ? '' : 's'} · start from the host's phone`
    : `${state.minPlayers - players.length} more crew needed`;
}
...
function renderReport() {
  ...
  const fatal = beat.kind === 'morning_vent';
  card.classList.toggle('is-fatal', fatal);
  card.classList.toggle('is-quiet', !fatal);
  el('report-label').textContent = `Cycle ${state.cycle} · overnight`;
  el('report-line').textContent = fatal ? `${beat.victim.name} was vented` : 'Nobody was vented';
  el('report-sub').textContent = fatal
    ? `${beat.victim.colour?.name || ''} · ${beat.victim.roleLabel}`
    : 'The crew is intact. Somebody is disappointed.';
...
function renderVote() {
  ...
  el('vote-count').textContent = `${state.votesIn} of ${alive.length} ballots in · ${state.voteNeeded} to eject`;
}
...
function renderTally() {
  ...
  el('tally-need').textContent = `${vote.needed} needed · ${vote.abstained.length} did not vote`;
...
function renderRevenge() {
  ...
  el('revenge-title').textContent = `${officer.name} was Security`;
}
...
function renderOver() {
  ...
  if (!winner) return;
  const crew = winner.side === 'crew';

  const over = document.querySelector('.over');
  over.classList.toggle('is-crew', crew);
  over.classList.toggle('is-saboteurs', !crew);
  el('over-title').textContent = crew ? 'Crew survives' : 'Saboteurs take the ship';
  el('over-label').textContent = `Cycle ${state.cycle} · final report`;
 ...
 *
 * A paused clock is genuinely frozen — the server stops counting, so this has
...
function tickClock() {
  ...
  el('hud-clock').textContent = !running ? '' : state.holding ? '•••' : formatClock(left);
...
// ---------------------------------------------------------------------------
// This screen's own controls — sound, and which voice reads the lines
// ---------------------------------------------------------------------------
 ...
 * been interacted with, and every game control now lives on the host's phone —
...
// Two taps: one to open the menu, one to confirm — this can end a game that is
 ...
 * of a room full of people. Two things get reported — the server refusing, and
...
function sendControl(name) {
  ...
  const giveUp = setTimeout(() => {
    if (answered) return;
    status.textContent = 'No answer from the ship. If the server has been running since before this '
      + 'control was added, restart it — then reload this screen.';
