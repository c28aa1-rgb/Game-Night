 ...
 * promise when it stops sounding, and the edit advances off the back of that —
 ...
 *
 * If any of this is not ready — clips still downloading, decode failed, no
 * codec — `ready` reports false and the caller falls back to the drawn opening
 * that shipped before this existed. A cinematic that might not play is not
 * allowed to be the only opening.
 */

window.MaydayFilm = (() => {
  const FILM = '/mayday/film';
  const VOX = '/mayday/vox';
   ...
   * the scene is doing. `overlay` screen-blends an element clip on top —
   * atmosphere leaving, or debris — and `shake` is the compartment failing.
   */
  const CUTS = [
   ...
   * part that failed — and, critically, never appends a second set of video
  ...
  function preload(container) {
    ...
    loading = (async () => {
       ...
       * into before any gesture — it is only playback that waits — but not all
      ...
      // elements are survivable — they are overlays, not the picture.
  ...
  /**
   * Run the crew's half of the opening.
   *
   ...
   * or a skip took the screen — the caller should treat that as "not my
  ...
  async function play(options = {}) {
    ...
    SFX()?.intro.start();
    // One duck, held for the whole sequence — see the note in sfx.js voice.play.
    SFX()?.duck(true);
  ...
  return {
    ...
    /** True once every line has audio — pictures can be ready before voices. */
