 ...
 *
 *   bed     the ship itself â€” a drone under the night, a pulse under the day.
...
window.MafiaSFX = (() => {
  ...
  function build() {
     ...
     * The town.
     *
     * The opening's dialogue is pre-rendered audio rather than synthesis â€” see
     * scripts/build-voices.js for why â€” but it still belongs in this graph and
     ...
     * needs no audio file â€” an impulse response is just noise with an
  ...
  const D1 = 36.71;
  const D2 = 73.42;
  const CHORD = [146.83, 174.61, 220];      // D3 F3 A3 â€” the ship, minor
  ...
  /**
   * The town's dialogue in the opening.
   *
   ...
   * on the dialogue rather than on a timer â€” a slow network or a long line
  ...
  const voice = {
    ...
    async load(urls = []) {
      ...
      await Promise.all(urls.map(async (url) => {
        ...
        try {
          ...
          // A missing line is survivable â€” the caption still carries it.
        }
      }));
      return ready;
    },
     ...
     * loaded â€” the cinematic then falls back to holding for the caption's own
    ...
    play(url) {
      ...
      // A little of the room, so the town sound like they are somewhere.
      const send = ctx.createGain();
       ...
       * apart, and a duck ramp is longer than that â€” ducking per line would
  ...
  const api = {
    ...
    /** 0â€“1. Nudges the ambient pulse as a phase's clock runs down. */
    setIntensity(value) {
    ...
    duck(on) {
      ...
      if (!ctx) return;
      applyBedGain();
      // The score ducks too, but less â€” it is meant to be heard under the
     ...
     * does to the alarm â€” a 12ms duck rather than an instant zero, because an
     ...
     * again from much further away. The echo is the whole point â€” it is the
    ...
    /**
     * The Vigilante Officer's sidearm. Loud, short, and over before the room has
     * finished flinching.
     */
    gunshot() {
      if (!live()) return;
    ...
    /** The Godfather's tell: the suit stuttering before it gives up the lie. */
    glitch() {
      if (!live()) return;
      for (let i = 0; i < 6; i++) {
