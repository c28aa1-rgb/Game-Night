 ...
 *
 *   base sequence   â† HOW they died
 *   reveal flourish â† WHAT they were
 *
 * There are three ways to be vented, three ways to be ejected by vote, one
 * sidearm, and one false alarm â€” and six role flourishes on the end of any of
...
window.MafiaCutscene = (() => {
   ...
   * the mafia, an instrument for each officer, and â€” deliberately â€”
   * nothing dramatic at all for plain town.
   */
  const GLYPHS = {
    mafia: {
      tint: 'var(--klaxon)',
  ...
  // The Godfather wears the town badge until the suit stops being able to hold it.
  GLYPHS.godfather = { ...GLYPHS.mafia, label: 'Godfather' };
  ...
  async function ventAirlock({ stage, figure, speed, sfx, wait }) {
    ...
    stage.innerHTML = `
      ...
      <div class="airlock">
        ...
        <p class="airlock__label readout">Airlock 4 Â· cycling</p>
      </div>
      ${streaks()}`;
  ...
  async function ventBreach({ stage, figure, speed, sfx, wait }) {
    ...
    stage.innerHTML = `
      ...
      <div class="breach">
        ...
        <p class="breach__label readout">Hull section 9 Â· integrity lost</p>
      </div>
      ${streaks()}
      <div class="blast"></div>`;
  ...
  async function ventShaft({ stage, figure, speed, sfx, wait }) {
    ...
    stage.innerHTML = `
      <div class="deck">
        ...
        <p class="shaft__label readout">Maintenance shaft 2 Â· open</p>
      </div>`;
  ...
  async function voteTrapdoor({ stage, figure, speed, sfx, wait }) {
    ...
    stage.innerHTML = `
      <div class="deck">
        ...
        <p class="trap__label readout">Disposal chute Â· released</p>
      </div>
      <div class="spotlight"></div>`;
  ...
  async function voteCargo({ stage, figure, speed, sfx, wait }) {
    ...
    stage.innerHTML = `
      <div class="bay">
        ...
        <p class="bay__label readout">Cargo bay 1 Â· depressurising</p>
      </div>
  ...
  // --- The Vigilante Officer's parting shot ------------------------------------
  ...
  async function flourish({ layer, role, sfx, wait, onTag }) {
    const glyph = GLYPHS[role] || GLYPHS.town;
    const isGodfather = role === 'godfather';
    const hostile = role === 'mafia' || isGodfather;
    ...
    if (isGodfather) {
      ...
      layer.innerHTML = `
        <div class="flourish__glyph is-glitching">${GLYPHS.town.svg}</div>
        <p class="flourish__label display">Town</p>`;
...
<svg class="ship" viewBox="0 0 720 260" fill="none">
  ...
  const SYSTEMS = [
    ...
    ['Town manifest', 'incomplete'],
  ];

  /**
   * The drawn opening. None of it advances on its own â€” introScene() is called
   ...
   *           cannot play â€” clips still downloading, no codec, sound never
   *           unlocked â€” and it is a complete opening in its own right, which
   ...
   *           ship's three closing lines, once the town are all dead and the
   *           survivors are the people in the room.
   */
  function playIntro(town = [], options = {}) {
    if (!root) return;
    stop();
    const short = !!options.short;
    current = { intro: true, short };

    const suits = town.slice(0, 12).map((player) => `
      <div class="lineup__member">
        ${A.svg({ hex: player.colour?.hex, size: town.length > 8 ? 84 : 108 })}
    ...
    const story = short ? '' : `
        <div class="intro__scene intro__scene--ship is-live">
          ...
          <p class="intro__slug readout">Automated distress relay Â· cycle 4,102 Â· no reply</p>
        </div>
        ...
        <div class="intro__scene intro__scene--wound">
          ...
          <p class="intro__slug readout">Reactor shielding Â· opened from inside</p>
        </div>

        <div class="intro__scene intro__scene--systems">
          <p class="readout intro__eyebrow">Kestrel Â· systems report</p>
          <ul class="systems">
...
${story}
        <div class="intro__scene intro__scene--town${short ? ' is-live' : ''}">
          <p class="readout intro__eyebrow">Surviving town</p>
          <div class="lineup">${suits}</div>
        </div>

        <div class="intro__scene intro__scene--title">
          <div class="shockwave"></div>
          <h1 class="display intro__mark">Mafia</h1>
     ...
     */
    if (!short) {
      window.MafiaSFX?.intro.start();
      window.MafiaSFX?.intro.scene(0);
    }
  }
  ...
  function introScene(index) {
     ...
     * head count both play over the roll call â€” the survivors need a moment to
     * be looked at â€” and the title lands on the line about the suit.
     */
    if (root.classList.contains('cut--intro-short')) {
      ...
      if (first && target.classList.contains('intro__scene--title')) {
        window.MafiaSFX?.titleHit?.();
      }
      // One of the suits in the line-up is lying, and says so for a frame.
      if (index >= 1 && target.classList.contains('intro__scene--town')) {
        ...
        if (members.length && !target.querySelector('.lineup__member.is-wrong')) {
          ...
          window.MafiaSFX?.glitch();
        }
      }
      return;
    }

    // Six narrated lines over five scenes: the last two lines share the town
    ...
    if (changed) window.MafiaSFX?.intro.scene(sceneIndex);
    ...
    if (changed && target.classList.contains('intro__scene--systems')) {
      ...
      rows.forEach((row, i) => setTimeout(() => {
        ...
        window.MafiaSFX?.systemFail();
      }, 520 * i));
    }

    // The last line is the one about a suit that is lying. One of them flickers.
    if (target.classList.contains('intro__scene--town') && index >= 4) {
      const members = target.querySelectorAll('.lineup__member');
   ...
   *
   * The overlay covers everything, including the status bar â€” which is where
  ...
  async function play(options = {}) {
    ...
    try {
      ...
      // A newer beat took the screen. Nothing to clean up â€” it already reset
  ...
  /** Abandon whatever is on screen â€” a new beat has arrived. */
  function stop() {
    current = null;
    if (root?.classList.contains('cut--intro')) window.MafiaSFX?.intro.stop();
